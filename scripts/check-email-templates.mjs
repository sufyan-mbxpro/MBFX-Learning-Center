...
// upstream of @repo/contracts, so `seed.ts` can never read the registry.
//
...
/** Keys of the `EMAIL_TEMPLATES = [ ... ]` array in the seed. */
export function seedKeys(source) {
  const block = source.match(/const EMAIL_TEMPLATES = \[([\s\S]*?)\n\] as const;/);
  if (!block) return [];
