-- changes-39 (owner): turn off the `facts` band in an existing
-- `home.sections`.
--
...
-- `facts` entry exists and is not already disabled.
UPDATE `settings`
SET `value` = JSON_SET(
      `value`,
      REPLACE(
        JSON_UNQUOTE(JSON_SEARCH(`value`, 'one', 'facts', NULL, '$[*].key')),
        '.key',
        '.enabled'
      ),
      FALSE
...
WHERE `key` = 'home.sections'
  AND JSON_SEARCH(`value`, 'one', 'facts', NULL, '$[*].key') IS NOT NULL
  AND JSON_EXTRACT(
        `value`,
        REPLACE(
          JSON_UNQUOTE(JSON_SEARCH(`value`, 'one', 'facts', NULL, '$[*].key')),
