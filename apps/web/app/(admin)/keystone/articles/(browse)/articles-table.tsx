      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuGroup>
            ...
            <DropdownMenuItem
              render={
                <Link href={`/keystone/articles/${row.id}`}>
...
export function ArticlesTable({
  ...
  const columns = useMemo<ColumnDef<ArticleRow>[]>(
    () => [
      {
        ...
        cell: ({ row }) => (
          <div className={`flex max-w-72 flex-col ${row.original.deleted ? "opacity-60" : ""}`}>
            <Link
              href={`/keystone/articles/${row.original.id}`}
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <RowSwitch
            ...
            onToggle={(next) => setArticleActiveAction(row.original.id, next)}
          />
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <RowSwitch
            ...
            onToggle={(next) => setArticleFeaturedAction(row.original.id, next)}
          />
        ),
      },
      {
    ...
    [labels, categories, canCreate, canDelete],
  );
