...
/*
 * SendGrid is its own driver since ADR-152 (changes-49 had stored it as an
 * SMTP row): it sends over the v3 Web API, because `sandbox_mode` exists there
 * and on no SMTP path. The API key still goes in the password slot, sealed
 * exactly as the SMTP password is (ADR-078 #3) — no second secret store:
 * security.md #10 names three sealed secrets and this is not a fourth.
 */
...
export interface EmailTransportLabels {
  ...
  sandbox: string;
  sandboxHint: string;
  sandboxOnTitle: string;
  sandboxOnBody: string;
  host: string;
  port: string;
  security: string;
  securityNone: string;
...
export function EmailTransportForm({
  ...
  const [driver, setDriver] = React.useState<EmailTransportView["driver"]>(transport.driver);
  const isSendgrid = driver === "SENDGRID";
  ...
  const [sandboxMode, setSandboxMode] = React.useState(transport.sandboxMode);
  ...
  const values = React.useMemo(
    () =>
      isSendgrid
        ? {
            driver,
            // The schema requires a security value on every driver; SendGrid
            // is HTTPS and reads none of the SMTP columns.
    ...
    [driver, isSendgrid, host, port, security, username, password, sandboxMode],
  );
  ...
  return (
    ...
    >
      ...
      {/* ADR-152: SendGrid validates the whole request and delivers nothing.
          Absent on SMTP rather than disabled — no SMTP server has the mode. */}
      {isSendgrid && (
        <SwitchRow orientation="horizontal">
          <Switch checked={sandboxMode} onCheckedChange={setSandboxMode} />
          <FieldContent>
            <FieldLabel>{labels.sandbox}</FieldLabel>
            <FieldDescription>{labels.sandboxHint}</FieldDescription>
          </FieldContent>
        </SwitchRow>
      )}

      {/* The SAVED state, not the switch: a sandboxed transport is one that
          silently delivers nothing, including password resets. */}
      {transport.driver === "SENDGRID" && transport.sandboxMode && (
        <Alert variant="warning">
          <AlertTitle>{labels.sandboxOnTitle}</AlertTitle>
          <AlertDescription>{labels.sandboxOnBody}</AlertDescription>
        </Alert>
      )}
