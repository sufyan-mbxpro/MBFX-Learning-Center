...
//
// `provider-form.tsx` in `/keystone/market` is the form this copies, down to the
...
export function AiProviderForm({
  ...
  const save = () => {
    ...
    run(
      async () => {
        ...
        if (!provider.id) router.replace(`/keystone/settings/ai/providers/${id}`);
      },
      {
  ...
  const remove = () => {
    ...
    run(() => deleteAiProviderAction(provider.id!), {
      ...
      onDone: () => router.push("/keystone/settings/ai/providers"),
    });
  };
