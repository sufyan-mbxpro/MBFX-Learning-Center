...
export default async function AiFeaturesPage() {
  ...
  return (
    <>
      <p className="text-sm text-muted-foreground">{tAi("featuresDescription")}</p>
      <div className="flex flex-col gap-6">
        {/* `alt_text` has no screen that calls it since ADR-153, so a switch
            for it would save and change nothing (code-style.md #28). */}
        {cards
          .filter((card) => card.key !== "alt_text")
          .map((card) => {
