...
// The Usage tab of Settings → AI (changes-51; it was `/keystone/ai`'s landing
// screen). Everything Phase 2 adds reports into it.
//
// **It is live** (changes-51: "check the usage & stats … will work in real
// time"). Every call is metered in the run's own `finally`, in one transaction
// that writes the row, the daily rollup and the period counter, and nothing
// here is cached — so a reload is current to the last call. `LiveRefresh`
// makes that reload happen by itself while the tab is open.
//
...
export default async function AiUsagePage({ searchParams }: PageProps<"/keystone/settings/ai/usage">) {
  ...
  return (
    <>
      <p className="text-sm text-muted-foreground">{tAi("description")}</p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <LiveRefresh
          ...
          labels={{
            updated: tAi("liveUpdated", { time: "{time}" }),
            refresh: tAi("liveRefresh"),
          }}
        />
      </div>
