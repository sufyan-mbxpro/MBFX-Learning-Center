    ...
    <div className="flex justify-end">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem
            render={<Link href={`/keystone/website/pages/${row.id}`}>{labels.edit}</Link>}
          />
          <DropdownMenuItem
            onClick={() =>
              run(async () => {
                const id = await duplicatePageAction(row.id);
                router.push(`/keystone/website/pages/${id}`);
              })
            }
          >
            {labels.duplicate}
    ...
    <Table>
      ...
      <TableBody>
        {rows.map((row) => (
          <TableRow key={row.id}>
            <TableCell>
              <Link href={`/keystone/website/pages/${row.id}`} className="font-medium hover:underline">
