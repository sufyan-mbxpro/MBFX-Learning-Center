...
export function CategoryDialog({
  ...
  locale: string;
  /**
   * Called with the saved row's id and name once the action succeeds. The
   * topic editor's inline "New category" (ADR-144 §2) uses it to add the row
   * to its dropdown and select it without reloading the page.
   */
...
function CategoryForm({
  ...
  const submit = () => {
    ...
    run(
      async () => {
        const id = await saveVideoCategoryAction(payload);
        onSaved?.({ id, name: payload.translation.name });
      },
      {
        successMessage: target
          ? t("videoCategoryManager.saved")
          : t("videoCategoryManager.created"),
        onDone: onClose,
      },
    );
  };
