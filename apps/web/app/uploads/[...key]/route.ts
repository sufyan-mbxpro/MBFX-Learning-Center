...
//
// `?download=1` forces the attachment for any kind — see contentDispositionFor.
//
...
export async function GET(request: Request, { params }: RouteContext<"/uploads/[...key]">) {
  // A catch-all since ADR-144 §3: a key is `<category>/<random>.<ext>` now,
  // and every flat key stored before that still resolves at its old URL. The
  // segments are joined back into the one key string and handed to core,
  // whose OBJECT_KEY_PATTERN is the whole gate — no `..`, no backslash, no
  // extra segment, no prefix outside MEDIA_CATEGORIES. Nothing here parses a
  // path, so there is nothing here to get wrong.
  const file = (await params).key.join("/");
  ...
  // `?download=1` is the media library's Download button (changes-32). A
  // query flag rather than a second route: the bytes, the Range handling and
  // the cache headers are all identical, and only the disposition differs.
  const download = new URL(request.url).searchParams.get("download") === "1";
  const disposition = contentDispositionFor(meta.kind, meta.fileName, download);
