...
function ctx(file: string) {
  return { params: Promise.resolve({ key: file.split("/") }) };
}

describe("GET /uploads/[...key] — full responses", () => {
...
describe("GET /uploads/[...key] — Range requests (ADR-034 §1)", () => {
...
describe("GET /uploads/[...key] — a document downloads, a video plays (ADR-034 §1)", () => {
...
describe("GET /uploads/[...key] — a partial request never materialises the object", () => {
...
describe("GET /uploads/[...key] — a category-prefixed key (ADR-144 §3)", () => {
  it("joins the segments back into ONE key and lets core decide whether it is valid", async () => {
    readStoredFileMeta.mockClear();
    const res = await GET(req(), ctx("news/unknown.mp4"));
    expect(res.status).toBe(404);
    expect(readStoredFileMeta).toHaveBeenCalledWith("news/unknown.mp4");
  });

  it("passes a traversal attempt to core verbatim, where the key pattern refuses it", async () => {
    readStoredFileMeta.mockClear();
    const res = await GET(req(), { params: Promise.resolve({ key: ["..", "secret.mp4"] }) });
    expect(res.status).toBe(404);
    expect(readStoredFileMeta).toHaveBeenCalledWith("../secret.mp4");
  });
});
